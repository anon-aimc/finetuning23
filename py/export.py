"""
ANON Python implementation
2023 ANON

export the model to a .ts file, loadable by the ANON~ Max object
"""
...
class ExportANON(nn_tilde.Module):
...
if __name__ == '__main__':
    ...
    m = ExportANON(pretrained=pretrained, params=list(pretrained.parameters()))
    
    if not test:
        m.export_to_ts('../help/ANON.ts') # TODO: make this a command line argument
        print("Exported model to ../help/ANON.ts")
    else:
        # test_toy(m)
        test_gmd(m)
