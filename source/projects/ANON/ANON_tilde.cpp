...
class ANON : public object<ANON>, public vector_operator<> {
public:
  ...
  message<> maxclass_setup{
      ...
      [this](const c74::min::atoms &args, const int inlet) -> c74::min::atoms {
        cout << "ANON~ v" << VERSION << " - 2023 ANON" << endl;
  ...
  queue<> perform_threaded { this,
    MIN_FUNCTION {
      ...
      if (m_use_thread && !done_playing && in_notes.size()) {
        m_compute_thread = std::make_unique<std::thread>(&ANON::vectorToModel, this, in_notes);
...
void ANON::loadFinetuned(std::string path) {
...
void ANON::initialiseScore() {
  score.clear();
  score.reserve(MAX_SCORE_LENGTH);
}

ANON::ANON(const atoms &args)
...
bool ANON::has_settable_attribute(std::string attribute) {
...
void ANON::parseTimeEvents(MidiFile &midifile) {
...
void ANON::vectorToModel(std::vector<std::array<double, IN_DIM>> &v) {
...
double ANON::computeNextNoteTimeMs() {
...
bool ANON::incrementPlayIndexes() {
...
void ANON::processLiveOnsets(audio_bundle input) {
  int location = -1;
...
void ANON::operator()(audio_bundle input, audio_bundle output) {
...
void ANON::perform(audio_bundle input, audio_bundle output) {
